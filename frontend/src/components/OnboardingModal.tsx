...
export default function OnboardingModal({ onClose }: Props) {
  ...
  useEffect(() => {
    const handler = () => setInstallable(!!deferredPrompt)
    window.addEventListener("beforeinstallprompt", handler)
    return () => window.removeEventListener("beforeinstallprompt", handler)
  }, [])

  const mobileInstallable = isMobile && installable
  const baseSteps = 3
  const totalSteps = mobileInstallable ? baseSteps + 1 : baseSteps
    ...
    if (step === 4) {
      return (
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div>
            <h2 className="text-xl font-bold">Install Neco</h2>
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
              Add Neco to your home screen for quick access. Works offline and loads instantly.
            </p>
          </div>
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
      <div className="relative w-full max-w-sm rounded-2xl border border-border bg-card shadow-xl animate-fade-in">
        ...
        <div className="flex items-center justify-between border-t border-border px-6 py-3">
          ...
          ) : step === totalSteps && mobileInstallable ? (
            <Button size="sm" onClick={handleInstall}>
              Install <Download className="ml-1 h-3 w-3" />
            </Button>
          ) : (
